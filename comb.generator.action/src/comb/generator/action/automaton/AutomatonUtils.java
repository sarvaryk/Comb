...
public class AutomatonUtils {
	...
	public static boolean saveBuchi(final Element element, final Writer.Format format, final boolean showInfo, final boolean isNegated, final String filePath) {
		...
		try {
			...
			if(showInfo)
				InfoUtils.showMessageDialog("Generation of the B�chi-automaton was terminated, as it was not completed before timeout\nSee: " + e);
			e.printStackTrace();
		}
	...
	public static Optional<Automaton> getBuchi(final Element element, final boolean isNegated) {
		...
		try {
			...
			e.printStackTrace();
			InfoUtils.showMessageDialog("ERROR while generating the B�chi-automaton!\n" + e);
		}
